...
router.get(
  "/admin",
  protect,
  checkPermission("notifications.view_all"),
  async (req, res) => {
    try {
      const {
        employeeName,
...
router.put("/admin/read-all", protect, isAdmin, async (req, res) => {
  try {
...
router.put("/permissions/:userId", protect, isAdmin, async (req, res) => {
  try {
    ...
    const permission = Object.keys(permissions).find((key) => permissions[key]);

    const notification = new Notification({
      userId: req.params.userId,
      type: action === "grant" ? "permission_granted" : "permission_denied",
      title: action === "grant" ? "تم منحك صلاحية جديدة" : "تم سحب صلاحية منك",
